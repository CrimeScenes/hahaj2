...
local function GetTarget()
    ...
    elseif getgenv().Silent.Mode == "Regular" then
        return ClosestPlrFromMouse()  -- Get the closest player to the mouse if in 'Regular' mode
    end
end
...
local function getDynamicFov(targetPosition)
    ...
    if distance <= getgenv().Silent.Range.Close then
        return getgenv().Silent.Range.Close  -- Return Close FOV radius
    -- Check if the target is within the Mid range (50 to 100)
    elseif distance <= getgenv().Silent.Range.Mid then
        return getgenv().Silent.Range.Mid  -- Return Mid FOV radius
    -- Check if the target is within the Far range (100 to 200)
    elseif distance <= getgenv().Silent.Range.Far then
        return getgenv().Silent.Range.Far  -- Return Far FOV radius
    else
        return getgenv().Silent.Range.Far  -- Default to Far FOV if the target is beyond 200 units
    end
end
...
local function updateFov()
    ...
    if IsTargeting and TargetPlayer then  
        ...
        -- Check if FOV should be visible
        local fovVisible = getgenv().Silent.FovSettings.FovVisible
        
        -- Update the FOV display shape based on the dynamic radius
        if getgenv().Silent.FovSettings.FovShape == "Square" then
            ...
            for i = 1, 4 do
                ...
                line.Visible = fovVisible
                line.From = corners[i]
                line.To = corners[i % 4 + 1]
                line.Color = getgenv().Silent.FovSettings.FovColor
                line.Thickness = getgenv().Silent.FovSettings.FovThickness
                line.Transparency = getgenv().Silent.FovSettings.FovTransparency
                table.insert(FovParts, line)
            end
        elseif getgenv().Silent.FovSettings.FovShape == "Triangle" then
            local points = {
            ...
            Fov.Color = getgenv().Silent.FovSettings.FovColor
            Fov.Thickness = getgenv().Silent.FovSettings.FovThickness
            Fov.Transparency = getgenv().Silent.FovSettings.FovTransparency
            Fov.Filled = getgenv().Silent.FovSettings.Filled
            if getgenv().Silent.FovSettings.Filled then
                Fov.Transparency = getgenv().Silent.FovSettings.FillTransparency
            end
        end
    else
...
local function isPartVisible(part)
    -- Check if wall check is enabled in Silent mode
    if not getgenv().Silent.WallCheck then 
        return true
    end

    -- Get the origin from the camera position and the direction to the part
    local origin = Current_Camera.CFrame.Position
    local direction = (part.Position - origin).Unit * (part.Position - origin).Magnitude
    local ray = Ray.new(origin, direction)

    -- Cast the ray and check for hits while ignoring the local player and the part's parent
    local hit = Workspace:FindPartOnRayWithIgnoreList(ray, {Local_Player.Character, part.Parent})
    
    -- Return true if the part is visible or if the hit is the part itself (i.e., no obstacle hit)
...
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        if humanoid then
            ...
            -- Resolver logic and velocity adjustments based on Y velocity or magnitude
            ...
            elseif velocity.Magnitude > 50 and getgenv().Silent.Resolver then
                -- Adjust velocity for high speed and apply movement direction
                return humanoid.MoveDirection * 16
            else
                -- Revert to the previous prediction value
    ...
    return Vector3.new(0, 0, 0)  -- Return zero velocity if no humanoid is found
end
...
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        -- Check for vertical velocity and apply prediction adjustments if Resolver is enabled
        if velocity.Y < -30 and getgenv().Silent.Resolver then
            getgenv().Silent.Prediction = 0
            return velocity
        -- Check for high velocity and apply adjusted movement direction
        elseif velocity.Magnitude > 50 and getgenv().Silent.Resolver then
            local humanoid = player.Character:FindFirstChild("Humanoid")
            if humanoid then
                return humanoid.MoveDirection * 16
            ...
            -- Reset prediction if conditions are not met
            getgenv().Silent.Prediction = OldPredictionY
            return velocity
        end
    end
    ...
    if tool:IsA("Tool") then
        tool.Activated:Connect(function()
            if tick() - lastToolUse > 0.1 then  -- Debounce for 0.1 seconds
                ...
                local target
                if getgenv().Silent.Enabled and getgenv().Silent.Startup.mode == "target" then
                    target = TargetPlayer  -- Use the specific target if in 'target' mode
                elseif getgenv().Silent.Enabled and getgenv().Silent.Startup.mode == "normal" then
...
if getgenv().Adjustment.Checks.NoGroundShots == true then
    local function CheckNoGroundShots(Plr)
        if getgenv().Adjustment.Checks.NoGroundShots and Plr.Character:FindFirstChild("Humanoid") then
            -- Check if the player's humanoid is in Freefall
            if Plr.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
                pcall(function()
                    -- Get the part that needs velocity adjustment (assuming "HumanoidRootPart" here)
                    local TargetPart = Plr.Character:FindFirstChild("HumanoidRootPart")
                    if TargetPart then
                        -- Adjust the Y component of velocity if in freefall
                        local newVelocity = Vector3.new(TargetPart.Velocity.X, TargetPart.Velocity.Y * 0.2, TargetPart.Velocity.Z)
                        TargetPart.Velocity = newVelocity
                        TargetPart.AssemblyLinearVelocity = newVelocity
                    end
                end)
            end
        end
